...
import { Button, Chip, Link, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, Tooltip, User } from "@nextui-org/react";
...
export default function DashboardPage() {
  ...
  const [users, setUsers] = useState([{nombreCompleto: '',genero: 'M', grado:1,email:'',edad: 0,TAT:[],IDB:{puntaje: 0, 
    clasificacion: 0, 
    respuestas: [{sintoma: '', intensidad: 0}] }}])
  ...
  useEffect(() => {
      ...
      axios.get('/api/data/getTotalUsers')
      .then(response => {
        ...
        <main className="w-screen flex items-center justify-center">
          <div className="flex-col w-full items-center justify-center">
            <div className="w-full grid grid-flow-row place-items-center">
              <h3 className="text-success text-2xl py-4">Usuarios que ya completaron ambos test</h3>
              <p className="text-foreground text-xl mt-2 mb-6">Total: {users.length} de {totalUsers} usuarios completados</p>
              ...
              {users.map((item, count)=>{
                if(count<15){
                  ...
                  <div className="flex w-30 gap-4 p-5 border border-secondary">
                    ...
                    <div className="w-40 text-center">{item.nombreCompleto}</div>
              ...
              <Button download={'users.json'} className="my-6" as={Link} variant="ghost" color="warning" href={url}>Descargar documento.</Button>
            </div>  
          </div>
        </main>
      </>
