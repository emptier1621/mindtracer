...
function IdbQuestionCard(props: {
  ...
  const content = idbQuestions.map((item) => {
    if (props.quest === item.question) {
      return (
        <IdbQuestionItem
          question={props.quest}
          sintoma={item.sintoma}
          opciones={item.opciones}
          key={item.question}
          fSumbit={handleOnSubmit}
          loading={loading}
  ...
  if (props.quest === 22) {
    return (
      <div className="w-full md:px-24 my-4 px-4 h-full flex-col justify-center items-center ">
        ...
        <p className="px-4 md:px-24 text-justify ">
          ¡Has completado el Inventario de Depresión de Beck (BDI-II) con éxito!
          ¡Gracias por tu colaboración! Ahora, para seguir mejorando, te
          invitamos a participar en la siguiente evaluación. ¡No te la pierdas!
        </p>
        <div className="w-full flex items-center justify-center my-8">
          <Button as={Link} href="/dashboard" variant="ghost" color="warning">
            Ir al dashboard
          </Button>
        </div>
      </div>
